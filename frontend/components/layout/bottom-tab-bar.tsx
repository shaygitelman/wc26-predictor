  ...
  return (
    <nav
      className={cn(
        ...
        'bg-surface/95 backdrop-blur-md border-t border-border/80',
        'pb-safe',
      )}
    >
      {/* Gradient fade — blends nav into page content */}
      <div className="absolute -top-8 left-0 right-0 h-8 bg-gradient-to-b from-transparent to-surface/90 pointer-events-none" />
        ...
        {TABS.map(({ href, label, Icon }) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span className="absolute top-2 left-1/2 -translate-x-1/2 w-8 h-[3px] rounded-full bg-primary shadow-[0_0_12px_rgba(124,111,255,0.7)]" />
              )}
              <Icon
                className={cn(
