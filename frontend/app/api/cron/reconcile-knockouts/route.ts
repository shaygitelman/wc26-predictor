...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${BACKEND_URL}/cron/reconcile-knockouts`, {
      method:  'POST',
    ...
    if (!res.ok) {
      ...
      Sentry.captureMessage(`reconcile-knockouts backend error ${res.status}`, {
        level: 'error',
    ...
    if (data.status === 'error' || data.errors?.length) {
      Sentry.captureMessage('reconcile-knockouts completed with errors', {
