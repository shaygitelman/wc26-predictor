...
export default function LeaguesPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      setLeagues(prev => {
        const exists = prev.some(l => l.id === joined.id)
      ...
      <BottomSheet open={sheet === 'create'}>
        ...
        <div className="px-6 py-4 flex flex-col gap-3">
          <div>
            ...
            />
            {createError && (
              <p className="text-xs text-status-lost mt-1.5">{createError}</p>
            )}
          </div>
          ...
          >
            {createStatus === 'success' ? (
              <><Check className="size-4" strokeWidth={3} /> League Created!</>
      ...
      <BottomSheet open={sheet === 'join'}>
        ...
        <div className="px-6 py-4 flex flex-col gap-3">
          <div>
            ...
            />
            {joinError && (
              <p className="text-xs text-status-lost mt-1.5">{joinError}</p>
            )}
          </div>
          ...
          >
            {joinStatus === 'success' ? (
              <><Check className="size-4" strokeWidth={3} /> Joined!</>
            ) : joinStatus === 'loading' ? (
              'Joining…'
            ) : (
              'Join League'
